            ...
            foreach ($this->AllProducts as $product) {
                ...
                if(!empty($allProductsProduct->skus)){
                    foreach ($allProductsProduct->skus as $allPdctsPdctsku) {
                        $in_purchase = $allPdctsPdctsku->product->product->skus->where('id',$allPdctsPdctsku->product_sku_id)->first();
                        $purchase_code  = '';
                        if(!empty($allPdctsPdctsku->in_app_purchase))
                        {
                            $purchase_code = $allPdctsPdctsku->in_app_purchase;
                        }else{
                ...
                $allProducts[] = [
                    ...
                    "product" => !empty($allProductsProduct)  ? [
                        ...
                        "product" => [
                            ...
                            "unit_type_id" => ($allProductsProductProduct->unit_type && $allProductsProductProduct->unit_type->id) ? $allProductsProductProduct->unit_type->name : $allProductsProductProduct->unit_type_id,
