...
<head>
    <meta charset="UTF-8">
    <title>Tax Invoice - {{$order->package_code}}</title>
    <style>
        body {
        ...
        .seller-name { font-weight: bold; text-transform: uppercase; }
        
        .divider { border-top: 2px solid #000; margin: 10px 0; }
        .thin-divider { border-top: 1px solid #000; margin: 10px 0; }
        
        .address-box { vertical-align: top; width: 50%; }
        .address-title { font-weight: bold; font-size: 10px; margin-bottom: 5px; text-transform: uppercase; }
        
        .items-table { margin-top: 10px; }
        .items-table th { border-top: 2px solid #000; border-bottom: 2px solid #000; padding: 5px 2px; text-align: right; font-size: 10px; }
        .items-table th:nth-child(1), .items-table th:nth-child(2) { text-align: left; }
        .items-table td { padding: 5px 2px; vertical-align: top; text-align: right; }
        .items-table td:nth-child(1), .items-table td:nth-child(2) { text-align: left; }
        
        .total-row td { border-top: 1px solid #000; border-bottom: 2px solid #000; font-weight: bold; padding: 5px 2px;}
        
        .net-payable { text-align: right; padding: 10px 0; font-weight: bold; font-size: 14px; }
        .amount-words { text-align: right; font-weight: bold; font-size: 12px; }
        
        .footer { margin-top: 40px; }
        .footer td { vertical-align: top; }
        .signature { text-align: right; }
        .signatory-title { font-weight: bold; }
        .declaration { font-size: 9px; text-align: justify; margin-top: 20px;}
        
        .barcode { text-align: right; margin-top: 10px; }
    ...
    <table>
        <tr>
            <td class="header-left">
                <h1>Tax Invoice</h1>
                <div class="seller-name">{{ $seller_name }} @if(@$seller->SellerBusinessInformation->business_tax_id) GSTIN:{{$seller->SellerBusinessInformation->business_tax_id}} @endif</div>
                <div>{{ @$seller->SellerBusinessInformation->business_address1 ?: app('general_setting')->address }}</div>
                <div>{{ @$seller->SellerBusinessInformation->business_city }} {{ @$seller->SellerBusinessInformation->business_state }} {{ @$seller->SellerBusinessInformation->business_postcode }}</div>
            </td>
            <td class="header-right">
                <table class="metadata" style="width:100%; text-align:right;">
                    <tr><td width="60%">Inv. No.</td><td width="40%">{{ $order->package_code }}</td></tr>
                    <tr><td>Order #</td><td>{{ $order->order->order_number }}</td></tr>
                    <tr><td>Inv. Date</td><td>{{ $order->created_at->format('M d, Y') }}</td></tr>
                    <tr><td colspan="2" style="font-weight:bold;">ORIGINAL FOR RECIPIENT</td></tr>
                </table>
                <div class="barcode">
    ...
    <table>
        <tr>
            <td class="address-box">
                ...
                <div>{{ @$shipping_addr->getShippingCity->name ?? @$shipping_addr->getCity->name }}, {{ @$shipping_addr->getShippingState->name ?? @$shipping_addr->getState->name }} {{ $shipping_addr->shipping_postcode ?? $shipping_addr->postal_code }}</div>
            ...
            <td class="address-box">
                ...
                <div>{{ @$billing_addr->getBillingCity->name ?? @$billing_addr->getCity->name }}, {{ @$billing_addr->getBillingState->name ?? @$billing_addr->getState->name }} {{ $billing_addr->billing_postcode ?? $billing_addr->postal_code }}</div>
            </td>
        </tr>
    </table>
    ...
    <table style="width: 100%; margin-top: 20px;">
        <tr>
            <td style="width: 50%; vertical-align: top;">
                ...
                <table style="border: 1px solid #000; border-radius: 5px; width: 220px; border-collapse: separate; padding: 5px;">
                    <tr>
                        ...
                        <td style="vertical-align: middle; padding-left: 8px;">
                            <div style="font-size: 14px; font-weight: bold; margin-bottom: 2  px;">{{ single_price($grand_total) }}</div>
