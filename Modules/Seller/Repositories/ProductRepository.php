...
class ProductRepository {
    ...
    public function getRecomandedProduct($seller_id = null, $warehouse_id = null){
        $section = HomePageSection::where('section_name', 'more_products')->first();
        return $section->getApiProductByQuery($seller_id, $warehouse_id);
    }

    public function getTopPicks($seller_id = null, $warehouse_id = null){
    ...
    public function getActiveSellerProductBySlug($slug, $seller_slug = null){
        $query = SellerProduct::with('product','seller', 'reviews.customer', 'reviews.images', 'product.brand','product.categories','product.unit_type',
        'product.variations','product.skus', 'product.tags','product.gallary_images','product.relatedProducts.related_seller_products.product',
        'product.upSales.up_seller_products.product', 'product.crossSales.cross_seller_products.product','product.shippingMethods.shippingMethod')
        ->where('slug', $slug)
        ->where('status', 1);

        if ($seller_slug) {
            $query->whereHas('seller', function($q) use ($seller_slug) {
                $q->where('slug', $seller_slug);
            });
        }
