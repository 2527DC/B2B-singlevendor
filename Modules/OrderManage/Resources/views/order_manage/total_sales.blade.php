...
@section('styles')
    ...
    <style>
        ...
        #confirmedTable thead th.checkbox-column-main {
            color: #065f46;
            /* Emerald-800 */
        ...
        #confirmedTable thead th.checkbox-column-main::before {
            content: "SELECT";
            display: block;
            margin-bottom: 5px;
        }

        #confirmedTable tbody td.checkbox-column-main {
            background-color: #ffffff !important;
        }

        #confirmedTable tbody tr:hover td.checkbox-column-main {
            background-color: #ecfdf5 !important;
        }

        .checkbox-column-main input[type="checkbox"] {
            cursor: pointer;
            width: 18px;
            height: 18px;
            accent-color: #10b981;
                        ...
                        <div class="float-lg-right float-none pos_tab_btn justify-content-end">
                            <ul class="nav nav_list" role="tablist">
                                ...
                                @if (permissionCheck('confirmed_orders'))
                                    <li class="nav-item">
                                        <a class="nav-link" href="#order_confirmed_data" role="tab" data-toggle="tab" id="2"
                                            aria-selected="true">{{__('order.confirmed_orders')}}</a>
                                    </li>
                                @endif

                                @if (permissionCheck('completed_orders'))
                                    <li class="nav-item">
                                        <a class="nav-link" href="#order_complete_data" role="tab" data-toggle="tab" id="3"
                                            aria-selected="true">{{__('order.completed_orders')}}</a>
                                    </li>
                                @endif

                                @if (permissionCheck('pending_payment_orders'))
                                    <li class="nav-item">
                                        <a class="nav-link" href="#pending_payment_data" role="tab" data-toggle="tab" id="4"
                                            aria-selected="true">{{__('order.pending_payment_orders')}}</a>
                                    </li>
                                @endif

                                @if (permissionCheck('cancelled_orders'))
                                    <li class="nav-item">
                                        <a class="nav-link" href="#cancelled_data" role="tab" data-toggle="tab" id="5"
                                            aria-selected="true">{{__('order.cancelled_orders')}}</a>
                                    </li>
                                @endif

                                @if (permissionCheck('inhouse_orders'))
                                    <li class="nav-item">
                                        <a class="nav-link" href="#inhouse_order_data" role="tab" data-toggle="tab" id="6"
                                            aria-selected="true">{{__('order.inhouse_orders')}}</a>
                                    </li>
                                @endif
                        ...
                        <div class="tab-content">
                                        ...
                                        <div class="QA_table">
                                            ...
                                            <div class="" id="latest_order_div">
                                                <table class="table" id="orderPendingTable">
                                                    <thead>
                                                        <tr>
                                                            <th class="checkbox-column-all"><input type="checkbox"
                                                                    id="select_all_pending"
                                                                    title="{{__('order.select_all_orders')}}"></th>
                                    ...
                                    <div class="d-flex mb-3 align-items-center">
                                        ...
                                        <div class="mr-2">
                                            <button type="button" class="btn btn-primary"
                                                id="apply_bulk_delivery_confirmed">{{__('common.apply')}}</button>
                                        </div>
                                        @if(permissionCheck('shipping.invoice_generate'))
                                            <div class="mr-2">
                                                <button type="button" class="btn btn-info bulk_download_invoices_btn"
                                                    data-table-type="confirmed" title="Download Selected Invoices" disabled><i
                                                        class="fa fa-download"></i></button>
                                            </div>
                                        @endif
                                    </div>
                            ...
                            @if (permissionCheck('pending_payment_orders'))
                                <div role="tabpanel" class="tab-pane fade" id="pending_payment_data">
                                    <div class="box_header common_table_header ">
                                        <div class="main-title d-md-flex">
                                            <h3 class="mb-0 mr-30 mb_xs_15px mb_sm_20px">{{__('order.pending_payment_orders')}}
                                            </h3>
                                        </div>
                                    </div>
...
@push('scripts')
    <script>
                        ...
                        },
                        {
                            data: null, orderable: false, searchable: false, render: function (data, type, row) {
                                ...
                                if (row.packages && row.packages.length > 0) {
                                    row.packages.forEach(function (pkg) {
                                        packageIds.push(pkg.id);
                                    });
                                }
                                return '<input type="checkbox" class="bulk-select-pending" data-order-id="' + row.id + '" data-package-ids="' + packageIds.join(',') + '" />';
                    ...
                    columns: [
                        ...
                        },
                        {
                            data: null, orderable: false, searchable: false, render: function (data, type, row) {
                                ...
                                return '<input type="checkbox" class="bulk-select-confirmed" data-order-id="' + row.id + '" data-package-ids="' + packageIds.join(',') + '" />';
                            }, className: 'checkbox-column-item'
                        },
                        {
                            data: 'DT_RowIndex', name: 'id', render: function (data) {
        ...
        $(document).on('change', '#select_all_confirmed', function () {
            ...
            updateBulkDownloadButton('confirmed');
        });
        ...
        $(document).on('change', '.bulk-select-confirmed', function () {
            updateBulkDownloadButton('confirmed');
            // select_all_confirmed logic is partly missing for unchecking, but we ensure button state is updated here.
        });

        function updateBulkDownloadButton(type) {
        ...
        $(document).on('click', '.bulk_download_invoices_btn', function (e) {
            ...
            $('.bulk-select-' + type + ':checked').each(function () {
                let pIdsAttr = $(this).data('package-ids');
                if (pIdsAttr !== undefined && pIdsAttr !== null) {
                    let pIds = pIdsAttr.toString().split(',');
    ...
    <form id="bulk_invoice_download_form" method="POST" action="{{ route('shipping.bulk_invoice_download') }}"
