...
class Category extends Model
{
    ...
    public function sellerProductsAll()
    {
        ...
        $query = SellerProduct::where('status', 1)
        ...
        // ✅ Warehouse filter
        if (auth()->check() && auth()->user()->warehouse_id) {
            $query->where('user_id', auth()->user()->warehouse_id);
        }
    ...
    public function sellerProductWithPaginate()
    {
        ...
        $products = SellerProduct::with('product', 'reviews')
            ->where('status', 1)
            ->whereHas('product', function ($query) {
                return $query->whereHas('categories', function ($q) {
                    $q->where('category_id', $this->id);
        ...
        // ✅ Warehouse filter
        if (auth()->check() && auth()->user()->warehouse_id) {
            $products->where('user_id', auth()->user()->warehouse_id);
        }
