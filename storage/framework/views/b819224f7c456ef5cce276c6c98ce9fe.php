    ...
    (function($){
        ...
        $(document).ready(function() {
            ...
            $(document).on('submit', '#add_form', function(event) {
                ...
                $.ajax({
                    url: "<?php echo e(route('marketing.bulk-sms.store')); ?>",
                    type: "POST",
                    cache: false,
                    contentType: false,
                    processData: false,
            ...
            $(document).on('submit', '#edit_form', function(event) {
                ...
                formData.append('_token', "<?php echo e(csrf_token()); ?>");
                $.ajax({
                    url: "<?php echo e(route('marketing.bulk-sms.update')); ?>",
                    type: "POST",
                    cache: false,
                    contentType: false,
                    processData: false,
                    ...
                    success: function(response) {
                        ...
                        $("#submit_btn").prop('disabled', false);
                        $('#submit_btn').text('<?php echo e(__("common.save")); ?>');
                        $('#pre-loader').addClass('d-none');
                    },
                    error: function(response) {
                        if(response.responseJSON.error){
            ...
            $(document).on('submit', '#item_delete_form', function(event) {
                ...
                $.ajax({
                    url: "<?php echo e(route('marketing.bulk-sms.delete')); ?>",
                    type: "POST",
                    cache: false,
                    contentType: false,
                    processData: false,
            ...
            $(document).on('submit', '#testSMSForm', function(event) {
                ...
                if(phone != ''){
                    ...
                    $('#sms_send_btn').text('<?php echo e(__("marketing.sending")); ?>');
                    $('#error_phone').text('');
                    $('#pre-loader').removeClass('d-none');
                    var formElement = $(this).serializeArray()
                    var formData = new FormData();
                    ...
                    formData.append('_token', "<?php echo e(csrf_token()); ?>");
                    $.ajax({
                        url: "<?php echo e(route('marketing.bulk-sms.test-sms')); ?>",
                        type: "POST",
                        cache: false,
                        contentType: false,
                        processData: false,
                        ...
                        success: function(response) {
                            ...
                            $('#role_user').niceSelect();
                            $('#publish_date').datepicker();
                            toastr.success("<?php echo e(__('marketing.test_sms_has_been_send_successfully')); ?>", "<?php echo e(__('common.success')); ?>")
                            $("#sms_send_btn").prop('disabled', false);
                            $('#sms_send_btn').text('<?php echo e(__("common.send")); ?>');
                            $('#pre-loader').addClass('d-none');
            ...
            function showValidationErrors(formType, errors) {
                <?php if(isModuleActive('FrontendMultiLang')): ?>
                    $(formType + ' #error_title_<?php echo e(auth()->user()->lang_code); ?>').text(errors['title.<?php echo e(auth()->user()->lang_code); ?>']);
                    $(formType + ' #error_message_<?php echo e(auth()->user()->lang_code); ?>').text(errors['message.<?php echo e(auth()->user()->lang_code); ?>']);
                <?php else: ?>
            ...
            function smsDataTable(){
                $('#bulkSmsTable').DataTable({
                    ...
                    stateSave: true,
                    "ajax": ( {
                        url: "<?php echo e(route('marketing.bulk-sms.get-data')); ?>"
                    }),
...
<?php $__env->stopPush(); ?>
<?php /**PATH /var/www/html/mytestdhatri/Modules/Marketing/Resources/views/bulk_sms/components/_scripts.blade.php ENDPATH**/ ?>